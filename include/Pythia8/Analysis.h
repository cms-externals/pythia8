...
public:
  ...
  // Constructor.
  Thrust(int selectIn = 2) : select(selectIn), eVal1(), eVal2(), eVal3(), nFew(0) {}
...
public:
  ...
  ClusterJet(string measureIn = "Lund", int selectIn = 2, int massSetIn = 2,
    ...
    doReassign(reassignIn), yScale(), pTscale(), nJetMin(), nJetMax(), dist2Join(), dist2BigMin(), distPre(), dist2Pre(), nParticles(), nFew(0) {
...
public:
  ...
  CellJet(double etaMaxIn = 5., int nEtaIn = 50, int nPhiIn = 32,
    ...
    threshold(thresholdIn), eTjetMin(), coneRadius(), eTseed(), nFew(0), rndmPtr(rndmPtrIn) { }
...
public:
  ...
  SlowJet(int powerIn, double Rin, double pTjetMinIn = 0.,
    ...
    useStandardR(useStandardRin), origSize(), clSize(), clLast(), jtSize(), iMin(), jMin(), dPhi(), dijTemp(), dMin() { isAnti = (power < 0); isKT = (power > 0);
